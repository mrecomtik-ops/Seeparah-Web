...
export function BookCard({
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card card-shadow transition-transform duration-200 hover:-translate-y-1 hover:card-shadow-lg"
    >
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-secondary">
        ...
        {book.access_type === "paid" && (
          <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-gold px-2.5 py-1 text-[11px] font-semibold text-gold-foreground">
            ...
            {book.subscription_price_usd}/mo
          </span>
        )}
      </div>
