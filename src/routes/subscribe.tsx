...
function SubscribePage() {
  ...
  const alreadySubscribed =
    !!book && (subsQuery.data ?? []).some((s) => s.book_id === book.id);
      ...
      <main className="mx-auto max-w-4xl px-4 pb-20 pt-10 sm:px-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground">
            Every book on Seeparah starts with a free opening page. Premium
            books continue with a small monthly subscription —{" "}
            {Math.round(AUTHOR_PAYOUT * 100)}% goes straight to the author.
          </p>
        </div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2">
          ...
          <div className="relative overflow-hidden rounded-2xl border-2 border-gold bg-card p-7 card-shadow-lg">
            ...
            <p className="mt-4 flex items-start gap-1.5 rounded-lg bg-secondary px-3 py-2 text-xs text-secondary-foreground">
              ...
              Stripe runs in test mode — no card is charged. Of ${price.toFixed(2)},
              the author receives ${(price * AUTHOR_PAYOUT).toFixed(2)} and
              Seeparah keeps ${(price * PLATFORM_COMMISSION).toFixed(2)} for
              translation and hosting.
            </p>
          </div>
        </div>
